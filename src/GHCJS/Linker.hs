...
linkerSettings = do
  ...
  return (LinkerSettings debug prof dedup verbose [] (Just "ghcjsld.log"), LinkCommand{output, inputs, paths, libs, relocatable, roots, runMain})
...
link set@LinkerSettings{lsVerbosity = verbose} comm = do
  ...
  case linked of
    ...
    Left WriteError       -> error "Exiting due to error writing output"
...
link' command@LinkCommand{output, inputs, paths, libs, relocatable, roots, runMain} = phase "Linker" $ do
  linkLog stdout Verbosity1 ("[linking] " ++ output)
  let rootMods = [] :: [Text]
  liftIO $ createDirectoryIfMissing True (takeDirectory output)
  libPaths <- getLibraries paths libs

  let ( oInputs, inputs' ) = partition ("o"  `isSuffixOf`) (filter (/= "") inputs)
      ( aInputs, inputs'') = partition ("a"  `isSuffixOf`) inputs'
      (jsInputs, remain  ) = partition (("js" `isInfixOf`) . takeExtensions) inputs''
  ...
  if relocatable
    then linkRelocatable output aInputs oInputs jsInputs
...
linkRelocatable output aInputs oInputs jsInputs = do
  ...
  jsModules <- liftIO $ forM jsInputs $ \js -> (JsSource js,) <$> BL.readFile js

  whenM (verbosity Verbosity1) . logMessage $ "Writing archive to: " ++ show output

  liftIO . BL.writeFile output . buildArchive (Meta $ concat metas) $ map go (oModules ++ aModules) ++ jsModules

  whenM (fmap not . liftIO $ doesFileExist output) $ logError "Archive write missing"
...
linkFiles output runMainFile roots oInputs aInputs jsInputs = phase "linkFiles" $ do
  ...
  whenM (verbosity Verbosity1) . logMessage $ "Writing javascript to: " ++ show output

  liftIO $ BL.writeFile output (mconcat $ javascript ++ [linked] ++ runMain)

  whenM (fmap not . liftIO $ doesFileExist output) $ logError "Javascript write missing" 
